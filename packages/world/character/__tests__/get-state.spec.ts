...
import { mockedCharacterDeps, peetData } from './mocks';

describe(Character.name, () => {
  describe('#getState', () => {
    it('в поле "position" возвращает текущую позицию актора в мире', () => {
      const engine = new MockedEngine();
      const character = new Character(peetData, { ...mockedCharacterDeps, engine });
    ...
    it('в поле "data" возвращает данные о персонаже', () => {
      const character = new Character(peetData, mockedCharacterDeps);
      const state = character.getState();
