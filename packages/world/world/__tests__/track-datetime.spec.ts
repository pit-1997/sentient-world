...
describe(`${World.name} - отслеживание даты`, () => {
  describe('начальное состояние', () => {
    it('дата начинается с 1', () => {
      const world = new World(getMockedWorldDeps());
      const state = world.getState();

      expect(state.dateTime.date).toBe(1);
    });
  });

  describe('интервал отслеживания', () => {
    it(`проверяет смену даты раз в ${constants.DATE_TRACKING_INTERVAL} мс`, () => {
      ...
      engine.resumeThreads(constants.DATE_TRACKING_INTERVAL - 1); // Ждём почти полный интервал - день не должен измениться

      expect(world.getState().dateTime.date).toBe(1);
      ...
      engine.resumeThreads(constants.DATE_TRACKING_INTERVAL - 1);
      ...
      expect(world.getState().dateTime.date).toBe(2);
    });
  });
  ...
  describe('переход на следующий день', () => {
    it('происходит когда время меняется с позднего вечера на раннее утро', () => {
      ...
      engine.resumeThreads(constants.DATE_TRACKING_INTERVAL);
    ...
    it('может происходить несколько раз подряд', () => {
      ...
      expect(world.getState().dateTime.date).toBe(3);
      ...
      expect(world.getState().dateTime.date).toBe(4);
    });
  });
